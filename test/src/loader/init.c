...
post_analyze_hook(ParseState *pstate, Query *query)
{
	if (ts_extension_is_loaded())
		elog(WARNING, "mock post_analyze_hook " STR(PG_PROMETHEUS_VERSION_MOD));
	...
	if (BROKEN && !creating_extension)
		elog(ERROR, "mock broken " STR(PG_PROMETHEUS_VERSION_MOD));
}
...
_PG_init(void)
{
	 ...
	 */
	ts_extension_check_version(PG_PROMETHEUS_VERSION_MOD);
	elog(WARNING, "mock init " STR(PG_PROMETHEUS_VERSION_MOD));
...
{
	elog(WARNING, "mock function call " STR(PG_PROMETHEUS_VERSION_MOD));
	PG_RETURN_VOID();
}
